...
function createButtons() {
  ...
  for (const key in line) {
    ...
    button.textContent = line[key].text;
    button.className = "line-button";
    button.style.borderColor = line[key].color;
    button.style.color = line[key].color;
    button.style.backgroundColor = "white";
    button.style.fontWeight = "Bold";
    buttonContainer.appendChild(button);

    // Show all stations information when line button is clicked
    button.addEventListener("click", async () => {
      await showAllStationsInfo(key);
    });
  }
}

async function fetchRealTimeSchedule(lineCode, stationCode) {
  const api = "https://rt.data.gov.hk/v1/transport/mtr/getSchedule.php";
  try {
    const response = await fetch(`${api}?line=${lineCode}&sta=${stationCode}`);
    const result = await response.json();
    // console.log(result);
    if (result.status === 1) {
      return result.data; // Return data if the result code is successful
    } else {
...
async function showAllStationsInfo(lineCode) {
  const infoBox = document.getElementById("info-box");

  infoBox.innerHTML = ""; // Clear previous info

  // Fetch schedule for all stations on the selected line
  for (const station of line[lineCode].sta) {
    const schedule = await fetchRealTimeSchedule(lineCode, station.code);
    // console.log(schedule, lineCode, station);

    displaySchedule(schedule, lineCode, station);
...
function displaySchedule(scheduleData, lineCode, stationName) {
  const infoBox = document.getElementById("info-box");
  const stationInfo = document.createElement("div");
  stationInfo.className = "station-info";
  stationInfo.style.backgroundColor = line[lineCode].color;

  stationInfo.innerHTML = `

  <h2>${stationName.name}</h2>
  <div>開出月台: ${
    scheduleData[`${lineCode}-${stationName.code}`].UP?.[0].plat
  }</div>
  <div>開出時間:${
    scheduleData[`${lineCode}-${stationName.code}`].UP?.[0].time
  }</div>
  <div>開出月台:${
    scheduleData[`${lineCode}-${stationName.code}`].DOWN?.[0].plat
  }</div>
  <div>開出時間:${
    scheduleData[`${lineCode}-${stationName.code}`].DOWN?.[0].time
  }</div>
  `;

  infoBox.appendChild(stationInfo);
}
...
// function selectLine() {
//   realTime(lineCode, staCode);
// }
// function append {}
// classList.Remove("information")
// info-box.appendChild(template)
// .filter(key === linecode)
